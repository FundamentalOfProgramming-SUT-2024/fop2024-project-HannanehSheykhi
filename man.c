...
void updateCodeButtonVisibility() {
    for (int i = 0; i < MAX_CODE_BUTTONS; i++) {
        if (codeButtons[i].isVisible) {
            ...
            if (timeElapsed > 30) {
                codeButtons[i].isVisible = 0; 
                mvprintw(MAP_HEIGHT-2, 2, "\t\t\t\t");  // Clear the line showing the code
            } else {
                mvprintw(MAP_HEIGHT-3, 2, "Generated Code: %d (Valid for %d seconds)", 
                         codeButtons[i].generatedCode, 30 - timeElapsed);
            }
            refresh();
        }
...
int main() {
    ...
    while (1) {
        ...
        // Call updateCodeButtonVisibility regularly
        time_t currentTime = time(NULL);
