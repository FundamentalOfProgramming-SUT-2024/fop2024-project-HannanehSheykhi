...
void updateVisibility() {
    // Loop through the visible area around the player
    for (int i = player.y - 5; i <= player.y + 5; i++) {
        for (int j = player.x - 5; j <= player.x + 5; j++) {
            // Ensure we are within map boundaries
            if (i >= 0 && i < MAP_HEIGHT && j >= 0 && j < MAP_WIDTH) {
                // Check if current cell is a window ('.') and surrounding cells are walls ('#')
                if (map[i][j] == '.' && map[player.y][player.x] == '#') {
                    // Skip marking visibility for windows with walls
                    continue;
                }
