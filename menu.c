...
void showNewGameMenu(int option);
void CreateUser();
void EnterAsGuest();
...
void showNewGameMenu(int option) {
    clear();
    attron(COLOR_PAIR(1) | A_BOLD);
    mvprintw(2, 10, "---New Game Menu---\n");
    attroff(COLOR_PAIR(1) | A_BOLD);
    attron(COLOR_PAIR(4));

    mvprintw(4, 10, (option == 0 ? "> " : "  "));
    printw("Enter as User\n");

    mvprintw(5, 10, (option == 1 ? "> " : "  "));
    printw("Enter as Guest\n");
...
void EnterAsGuest() {
    Player newGuest;
    clear();
    mvprintw(0, 0, "Enter as Guest");
    mvprintw(2, 0, "Enter Username: ");
    echo();
    getstr(newGuest.username);
    noecho();

    addPlayer(players, &playerCount, newGuest.username);
    mvprintw(4, 0, "You have entered as guest: %s", newGuest.username);
    refresh();
    getch();
}
...
int main() {
    ...
    while (1) {
        ...
        if (ch == KEY_UP && choice > 0) {
            ...
            if (choice == 0) {
                int newGameChoice = 0;
                showNewGameMenu(newGameChoice);
                while (1) {
                    int newGameCh = getch();
                    if (newGameCh == KEY_UP && newGameChoice > 0) {
                        newGameChoice--;
                    } else if (newGameCh == KEY_DOWN && newGameChoice < 2) {
                        newGameChoice++;
                    } else if (newGameCh == '\n') {
                        if (newGameChoice == 0) {
                            CreateUser();
                        } else if (newGameChoice == 1) {
                            EnterAsGuest();
    ...
    switch (choice) {
        case 0:
            // The New Game sub-menu is already handled above
            break;
        case 1:
            clear();
